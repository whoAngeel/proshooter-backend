...
async def create_weapon(
    ...
    if current_user.role != RoleEnum.ADMIN:
        raise HTTPException(
            ...
            detail="Solo administradores pueden crear armas",
        )
...
async def get_all_weapons(
    ...
    return weapon_service.get_all_weapons(
        skip, limit, active_only, weapon_type, caliber
    )
...
async def update_weapon(
    ...
    if current_user.role != RoleEnum.ADMIN:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Solo administradores pueden actualizar armas",
        )
...
async def delete_weapon(
    ...
    if current_user.role != RoleEnum.ADMIN:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Solo administradores pueden eliminar armas",
        )
    ...
    if error:
        if error == "WEAPON_NOT_FOUND":
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=error)
    return {"message": "Arma eliminada correctamente", "id": weapon_id}
...
async def add_compatible_ammunition(
    ...
    if current_user.role != RoleEnum.ADMIN:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Solo administradores pueden agregar municiones compatibles",
        )
...
async def remove_compatible_ammunition(
    ...
    if current_user.role != RoleEnum.ADMIN:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Solo administradores pueden eliminar municiones compatibles",
        )

    success, error = weapon_service.remove_compatible_ammunition(
