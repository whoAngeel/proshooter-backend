...
async def create_biometric_data(
    ...
    if error_code == "PESONAL_DATA_ALREADY_EXISTS":
...
async def update_biometric_data(
    ...
    personal_data, error_code = UserService.update_personal_data(db, current_user_id, data_in)
...
def add_biometric_data(data_in: UserBiometricDataCreate, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    user_id = current_user.id
...
def update_biometric_data( data_in: UserBiometricDataUpdate, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    user_id = current_user.id
