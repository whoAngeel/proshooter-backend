...
async def list_exercise_types(
    search: Optional[str] = Query(None, description="Término de búsqueda en nombre o descripcion"),
    difficulty: Optional[int] = Query(None, description="Filtrar por nivel de dificultad"),
    active_only: bool = Query(True, description="Mostrar solo los tipos de ejercicio activos"),
...
async def update_exercise_type(
    ...
    exercise_type_id: UUID = Path(..., description="ID del tipo de ejercicio a actualizar"),
    service: ExerciseTypeService = Depends(),
    current_user: dict = Depends(get_current_user)
):
    '''
    ...
    '''
    if current_user.role != RoleEnum.ADMIN:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="No tienes permiso para actualizar tipos de ejercicio")

    exercise_type, error = service.update_exercise_type(exercise_type_id, exercise_type_data)
...
async def delete_exercise_type(
    exercise_type_id: UUID = Path(..., description="ID del tipo de ejercicio a eliminar"),
    service: ExerciseTypeService = Depends(),
    soft_delete: bool = Query(True, description="Indica si se debe eliminar logicamente o no el tipo de ejercicio"),
    current_user: dict = Depends(get_current_user)
):
    '''
    ...
    '''
    if current_user.role != RoleEnum.ADMIN:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="No tienes permiso para eliminar tipos de ejercicio")
