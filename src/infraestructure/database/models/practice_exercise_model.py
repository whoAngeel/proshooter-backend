...
class PracticeExerciseModel(Base):
    ...
    session_id = Column(
        UUID(as_uuid=True),
        ForeignKey("individual_practice_sessions.id"),
        nullable=False,
    )
    ...
    ammunition_id = Column(
        UUID(as_uuid=True), ForeignKey("ammunition.id"), nullable=False
    )
    ...
    target_image_id = Column(
        UUID(as_uuid=True), ForeignKey("target_images.id"), nullable=True
    )
