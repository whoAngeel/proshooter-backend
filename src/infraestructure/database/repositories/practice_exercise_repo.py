...
class PracticeExerciseRepository:
    ...
    def create(db: Session, exercise_data: dict) -> PracticeExerciseModel:
        ...
        if exercise_data.get("ammunition_used", 0) > 0 and 'accuracy_percentage' not in exercise_data:
            exercise_data["accuracy_percentage"] = (exercise_data.get("hits", 0) / exercise_data['ammunition_used']) * 100
    ...
    def get_by_id(db: Session, exercise_id: UUID) -> Optional[PracticeExerciseModel]:
        return db.query(PracticeExerciseModel).options(
        ...
        ).filter(PracticeExerciseModel.id == exercise_id).first()

    @staticmethod
    def get_all(db: Session, skip: int = 0, limit: int= 100) -> List[PracticeExerciseModel]:
        return db.query(PracticeExerciseModel).options(
            joinedload(PracticeExerciseModel.exercise_type),
            joinedload(PracticeExerciseModel.session)
        ).order_by(desc(PracticeExerciseModel.created_at)).offset(skip).limit(limit).all()
    ...
    def get_by_session(db: Session, session_id: UUID) -> List[PracticeExerciseModel]:
        ...
        ).order_by(PracticeExerciseModel.created_at).all()

    @staticmethod
    def get_by_shooter(db: Session, shooter_id: UUID, skip: int = 0, limit: int=100) -> List[PracticeExerciseModel]:
        return db.query(PracticeExerciseModel).join(
            ...
            desc(IndividualPracticeSessionModel.date)
        ).offset(skip).limit(limit).all()

    @staticmethod
    def get_by_exercise_type(db: Session, exercise_type_id: UUID, skip: int = 0, limit: int = 100) -> List[PracticeExerciseModel]:
        return db.query(PracticeExerciseModel).options(
            joinedload(PracticeExerciseModel.session)
        ).filter(
            PracticeExerciseModel.exercise_type_id == exercise_type_id
        ).order_by(
            desc(PracticeExerciseModel.created_at)
        ).offset(skip).limit(limit).all()

    @staticmethod
    def get_by_weapon(db: Session, weapon_id: UUID, skip: int = 0, limit: int = 100)-> List[PracticeExerciseModel]:
        return db.query(PracticeExerciseModel).options(
            joinedload(PracticeExerciseModel.session),
            joinedload(PracticeExerciseModel.exercise_type)
        ).filter(
            PracticeExerciseModel.weapon_id == weapon_id
        ).order_by(
            desc(PracticeExerciseModel.created_at)
        ).offset(skip).limit(limit).all()

    @staticmethod
    def get_by_ammunition(db: Session, ammunition_id: UUID, skip: int = 0, limit: int = 100)->List[PracticeExerciseModel]:
        return db.query(PracticeExerciseModel).options(
            joinedload(PracticeExerciseModel.session),
            joinedload(PracticeExerciseModel.exercise_type)
        ).filter(
            PracticeExerciseModel.ammunition_id == ammunition_id
        ).order_by(
            desc(PracticeExerciseModel.created_at)
        ).offset(skip).limit(limit).all()

    @staticmethod
    def get_by_target(db: Session, target_id: UUID, skip: int = 0, limit: int = 100) -> List[PracticeExerciseModel]:
        return db.query(PracticeExerciseModel).options(
            joinedload(PracticeExerciseModel.session),
            joinedload(PracticeExerciseModel.exercise_type)
        ).filter(
            PracticeExerciseModel.target_id == target_id
        ).order_by(
            desc(PracticeExerciseModel.created_at)
        ).offset(skip).limit(limit).all()

    @staticmethod
    def get_by_distance(db: Session, distance: str, skip: int = 0, limit: int = 100)-> List[PracticeExerciseModel]:
        return db.query(PracticeExerciseModel).options(
            joinedload(PracticeExerciseModel.session),
            joinedload(PracticeExerciseModel.exercise_type)
        ).filter(
            PracticeExerciseModel.distance == distance
        ).order_by(
            desc(PracticeExerciseModel.created_at)
        ).offset(skip).limit(limit).all()
    ...
    def search_by_combined_criteria(
        ...
        if filter_params.get('exercise_type_id'):
            query = query.filter(
                PracticeExerciseModel.exercise_type_id == filter_params['exercise_type_id']
            )
        ...
        if filter_params.get("distance"):
            query = query.filter(PracticeExerciseModel.distance.ilike(f"%{filter_params['distance']}%"))

        if filter_params.get("min_accuracy") is not None and filter_params.get("max_accuracy") is not None:
            query = query.filter(
                between(
    ...
    @staticmethod
    def update(db: Session, exercise_id: UUID, exercise_data: dict)->Optional[PracticeExerciseModel]:
        exercise = db.query(PracticeExerciseModel).filter(
    ...
    @staticmethod
    def get_statistics(db: Session, shooter_id: Optional[UUID] = None) -> Dict[str, Any]:
        query = db.query(
        ...
        distance_stats = distance_stats.group_by(
            PracticeExerciseModel.distance
        ).order_by(
            desc('count')
        ).all()
        ...
        exercise_type_stats = exercise_type_stats.group_by(
            ExerciseTypeModel.name
        ).order_by(
            desc('count')
        ).all()
        ...
        weapon_stats = db.query(
            ...
            WeaponModel,
            PracticeExerciseModel.weapon_id == WeaponModel.id
        )
        ...
        weapon_stats = weapon_stats.group_by(
            WeaponModel.name
        ...
        return {
            ...
            "total_ammunition_used": stats.total_ammunition_used if stats.total_ammunition_used else 0,
            "total_hits": stats.total_hits if stats.total_hits else 0,
            "hit_percentage": (stats.total_hits / stats.total_ammunition_used * 100) if stats.total_ammunition_used and stats.total_ammunition_used > 0 else 0.0,
            "avg_reaction_time": float(stats.avg_reaction_time) if stats.avg_reaction_time else None,
            "by_distance": [{
                "distance": d.distance,
                "count": d.count,
    ...
    def get_performance_analysis_by_category(
        ...
        if category == "weapon":
            query = db.query(
                ...
                func.avg(PracticeExerciseModel.accuracy_percentage).label("avg_accuracy"),
                func.sum(PracticeExerciseModel.ammunition_used).label("total_ammunition"),
                func.sum(PracticeExerciseModel.hits).label("total_hits")
            ).join(
                WeaponModel,
                PracticeExerciseModel.weapon_id == WeaponModel.id
            ...
            query = db.query(
                ...
                TargetModel,
                PracticeExerciseModel.target_id == TargetModel.id
            )
        ...
        for row in results:
            ...
            if category in ["weapon", "target", "ammunition", "exercise_type"]:
                ...
                result_dict = {
                    ...
                    "hit_percentage": (row[4] / row[3] * 100) if row[3] and row[3] > 0 else 0.0
                }
