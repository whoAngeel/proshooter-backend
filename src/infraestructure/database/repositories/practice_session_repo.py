...
class PracticeSessionRepository:
    ...
    def get_all(db: Session, skip: int = 0, limit: int=100) -> List[PracticeSessionModel]:
        ...
        ).order_by(desc(PracticeSessionModel.date)).offset(skip).limit(limit).all()
    ...
    def get_by_shooter(db: Session, shooter_id: UUID, skip: int = 0, limit: int=100) -> List[PracticeSessionModel]:
        return db.query(PracticeSessionModel).options(
            ...
            desc(PracticeSessionModel.date)
    ...
    def search_by_term(db: Session, term: str) -> List[PracticeSessionModel]:
        return db.query(PracticeSessionModel).join(
            ShooterModel, PracticeSessionModel.shooter_id == ShooterModel.user_id
        ).filter(
            or_(
        ...
        ).order_by(desc(PracticeSessionModel.date)).all()
    ...
    def get_statistics(db: Session, shooter_id: UUID, period: Optional[str] = None) -> dict:
        ...
        if start_date:
            query = query.filter(between(PracticeSessionModel.date, start_date, end_date))
    ...
    @staticmethod
    def get_by_accuracy_range(db: Session, min_accuracy: float, max_accuracy: float,
                             shooter_id: Optional[UUID] = None) -> List[PracticeSessionModel]:
        query = db.query(PracticeSessionModel).filter(
            between(PracticeSessionModel.accuracy_percentage, min_accuracy, max_accuracy)
        )
