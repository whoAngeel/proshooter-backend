...
from src.infraestructure.database.repositories.practice_exercise_repo import PracticeExerciseRepository
from src.infraestructure.database.repositories.practice_session_repo import PracticeSessionRepository as IndividualPracticeSessionRepository
...
class PracticeExerciseService:
    ...
    def create_exercise(self, exercise_data: PracticeExerciseCreate) -> Tuple[Optional[PracticeExerciseRead], Optional[str]]:
        try:
            # verificar que los objetos relacionados existen
            session = IndividualPracticeSessionRepository.get_by_id(self.db, exercise_data.session_id)
            if not session:
                return None, "PRACTICE_SESSION_NOT_FOUND"

            exercise_type = ExerciseTypeRepository.get_by_id(self.db, exercise_data.exercise_type_id)
            ...
            ammunition = AmmunitionRepository.get_by_id(self.db, exercise_data.ammunition_id)
            if not ammunition:
    ...
    def get_exercise_by_id(self, exercise_id: UUID) -> Tuple[Optional[PracticeExerciseDetail], Optional[str]]:
        try:
    ...
    def get_session_exercises(self, session_id: UUID) -> Tuple[Optional[List[PracticeExerciseRead]], Optional[str]]:
    ...
    def udpate_exercise(self, exercise_id: UUID, exercise_data: PracticeExerciseUpdate) -> Tuple[Optional[PracticeExerciseRead], Optional[str]]:
        try:
            existing_exercise = PracticeExerciseRepository.get_by_id(self.db, exercise_id)
            ...
            if exercise_data.weapon_id or exercise_data.ammunition_id:
                is_compatible = WeaponRepository.check_compatibility(self.db, weapon_id, ammo_id)
                if not is_compatible:
                    return None, "WEAPON_AMMUNITION_NOT_COMPATIBLE"

            # convertir los datos a diccionario para actualizar el ejercicio
            exercise_dict = exercise_data.model_dump(exclude_unset=True, exclude_none=True)

            # actualizar el ejercicio en la base de datos
            updated_exercise = PracticeExerciseRepository.update(self.db, exercise_id, exercise_dict)
    ...
    def get_exercise_statistics(self, shooter_id: Optional[UUID] = None)->Tuple[Optional[PracticeExerciseStatistics], Optional[str]]:
        try:
    ...
    def get_performance_analysis(self, category: str, shooter_id: Optional[UUID] =None)-> Tuple[Optional[PerformanceAnalysis], Optional[str]]:
        try:
            valid_categories = ["weapon", 'target', 'ammunition', 'distance', 'exercise_type']
            if category not in valid_categories:
                return None, f"INVALID_CATEGORY: deberia ser uno de {', '.join(valid_categories)}"
            ...
            analysis_data = PracticeExerciseRepository.get_performance_analysis_by_category(
            ...
            analysis = PerformanceAnalysis(
                ...
                total_exercises=sum(item.get("exercises_count", 0) for item in analysis_data),
            )
    ...
    def _update_session_totals(self, session_id: UUID) -> None:
        ...
        try:
            ...
            # precision
            accuracy_percentage = (total_hits / total_shots * 100) if total_shots > 0 else 0.0
            ...
            IndividualPracticeSessionRepository.update(self.db, session_id, session_data)
        except Exception as e:
            self.db.rollback()
