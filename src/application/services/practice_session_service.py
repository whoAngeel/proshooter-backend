...
class PracticeSessionService:
    ...
    def get_all_sessions(self, filter_params: IndividualPracticeSessionFilter) -> IndividualPracticeSessionList:
        ...
        items = [IndividualPracticeSessionDetailLite.model_validate(session) for session in sessions]
    ...
    def update_session(
        ...
        try:
            ...
            # preparar los datos
            session_dict = session_data.model_dump(exclude_unset=True, exclude_none=True)
            ...
            if "total_shots_fired" in session_dict or "total_hits" in session_dict:
                total_shots = session_dict.get("total_shots_fired", existing_session.total_shots_fired)
                total_hits = session_dict.get("total_hits", existing_session.total_hits)

                if total_shots > 0:
                    session_dict["accuracy_percentage"] = (total_hits / total_shots) * 100
                else:
                    session_dict["accuracy_percentage"] = 0.0
            # actualiza la sesion
            updated_session = PracticeSessionRepository.update(self.db, session_id, session_dict)
    ...
    def get_shooter_statistics(self, shooter_id: UUID, period: Optional[str] = None) -> Tuple[Optional[IndividualPracticeSessionStatistics], Optional[str]]:
        try:
            ...
            stats = PracticeSessionRepository.get_statistics(self.db, shooter_id, period)

            # Convierte al esquema de estadísticas
            return IndividualPracticeSessionStatistics(
    ...
    def get_recent_sessions(self, shooter_id : Optional[UUID]=None, limit: int = 5)-> List[IndividualPracticeSessionDetailLite]:
        if shooter_id:
            sessions = PracticeSessionRepository.get_by_shooter(self.db, shooter_id, 0, limit)
        else:
            sessions = PracticeSessionRepository.get_all(self.db, 0, limit)
        return [IndividualPracticeSessionDetailLite.model_validate(session) for session in sessions]
